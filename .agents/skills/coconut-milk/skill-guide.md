# Coconut Milk — Framework Guide

Full API reference and conceptual documentation. For the quick-start card, see `SKILL.md`.
...
> over raw `coconut.call()`. See [generators.md](generators.md).

The `coconut` global object is injected at `document_start` — available before any page scripts run.
...
| 5 | **`View.html()` uses temp files** | Inline HTML is written to a temp file and served via `file://` URL |
...
| 12 | **`@command` annotation required** | Without it, `coconut generate` skips the function |
| 13 | **Lua paths must end with `;`** | When manipulating `package.path`, ensure there's a trailing `;` |
...
- **View names must be unique strings** — used for routing, navigation links, and lifecycle callbacks.
