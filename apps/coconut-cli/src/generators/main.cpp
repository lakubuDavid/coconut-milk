...
static constexpr struct {
  ...
  const char* returnType;  // TS type for the return value
    ...
    // ── Filesystem ──
    {"fs_read_text", "{ path: string }", "{ ok: boolean; data?: string; error?: string }"},
    {"fs_exists", "{ path: string }", "{ ok: boolean; exists?: boolean; error?: string }"},
    {"fs_write_text", "{ path: string; content: string }", "{ ok: boolean; error?: string }"},
    {"fs_resolve",
...
static std::string deriveModulePath(const std::string& filePath) {
  ...
  if (p.has_parent_path()) {
    ...
    if (!parentStr.empty() && parentStr != "." && parentStr.find('/') == std::string::npos &&
...
static bool needsRegeneration(
    const fs::path& srcPath, const std::string& outDir, const std::string& stem
) {
  std::error_code ec;
  auto            srcTime = fs::last_write_time(srcPath, ec);
  if (ec)
    return true;  // can't read source time, regenerate to be safe
...
static std::string buildParamsType(const coconut::generator::CommandDefinition& def) {
  ...
  if (def.parameters.size() == 1) {
    std::string t = coconut::generator::formatTypeOrPassthrough(
        def.parameters[0].type, coconut::generator::formatTypeTS
    );
    if (!t.empty() && t != "any")
      return t;
  ...
  for (size_t i = 0; i < def.parameters.size(); ++i) {
    ...
    out += ": ";
    out += coconut::generator::formatTypeOrPassthrough(
        def.parameters[i].type, coconut::generator::formatTypeTS
    );
  }
  out += " }";
  return out;
...
static std::string buildReturnType(const coconut::generator::CommandDefinition& def) {
  if (def.returnTypes.empty())
    return "any";
  return coconut::generator::formatTypeOrPassthrough(
      def.returnTypes, coconut::generator::formatTypeTS
  );
}

/// Emit the builtin + user command type maps into the aggregated commands.d.ts.
static void writeAggregatedDTS(
    std::ostream&                                             agg,
  ...
  for (const auto& d : userDefs) uniqueByName[d.name] = &d;
  ...
  if (uniqueByName.empty()) {
    ...
    agg << "type CoconutCommandName = BuiltinCommandName";
    for (const auto& name : userNames) agg << " | \"" << name << "\"";
    agg << ";\n";
  }
  agg << "\n";
}
...
namespace coconut::generator {
  ...
  int runGenerate(const std::string& cmdRoot, const std::string& outDir) {
    ...
    for (const auto& entry : fs::directory_iterator(cmdDir)) {
      ...
      if (name.size() > 6 && name.substr(name.size() - 6) == ".g.lua")
        continue;
      if (name.size() > 10 && name.substr(name.size() - 10) == ".g_mt.lua")
        continue;
    ...
    coconut::println("generate: {} command(s) in {} — {}", totalCommands, cmdRoot, summary);
