...
namespace coconut::generator {
      ...
      while (true) {
        ...
        if (cur.peek() == '|') {
          ...
          if (right.kind == Type::Kind::Union) {
            for (auto& a : right.alternatives) alts.push_back(std::move(a));
          } else {
  ...
  inline Type parseType(const std::string& input) {
    ...
    cur.skipWs();
    if (!cur.atEnd())
      throw TypeParseError("unexpected trailing content: '" + input.substr(cur.pos) + "'");
    return result;
  }
  ...
  namespace detail {
    ...
    void formatFunParamsLua(const std::vector<FunParam>& params, std::string& out) {
      out += '(';
    ...
    void formatFunParamsJS(const std::vector<FunParam>& params, std::string& out) {
      out += '(';
      for (size_t i = 0; i < params.size(); ++i) {
        if (i > 0)
          out += ", ";
    ...
    void formatFunParamsTS(const std::vector<FunParam>& params, std::string& out) {
      out += '(';
      for (size_t i = 0; i < params.size(); ++i) {
        if (i > 0)
          out += ", ";
