...
namespace coconut::bg_thread {
  ...
  std::expected<Context*, Error> create(App* app, Config* config) {
    ...
    bg->ctx->app       = app;
    bg->ctx->lua_state = app->lua_state;

    // ── Register the CoconutContext usertype ──────────────────────────
    // This must happen BEFORE loadBackgroundCommands() so that .g.lua
    // files can call ctx:bind() on the background thread.
    {
      auto& lua = *bg->lua_state;
    ...
    // ── Register thread-safe coconut.* APIs on the background state ──
    {
      auto& lua      = *bg->lua_state;
      lua["coconut"] = lua.create_table();

      using namespace coconut::modules;
      init_log(lua, ThreadKind::Background);
      init_json(lua, ThreadKind::Background);
      init_fs(lua, ThreadKind::Background);
      init_dialog(lua, ThreadKind::Background);
      init_notify(lua, ThreadKind::Background);
      init_clipboard(lua, ThreadKind::Background);
      init_store(lua, ThreadKind::Background);
  ...
  static void loadBackgroundCommands(App* app, Context* bg) {
    ...
    for (const auto& scanDir : dirsToScan) {
      ...
      for (auto& entry : std::filesystem::directory_iterator(scanDir)) {
        ...
        auto loadResult = bg->lua_state->script_file(path.string(), sol::script_pass_on_error);
        if (!loadResult.valid()) {
          sol::error e = loadResult;
          debug::warn(std::format("[bg] failed to load {}: {}", path.filename().string(), e.what())
          );
          continue;
        }
        ...
        if (!ret.is<sol::function>()) {
          debug::warn(std::format(
              "[bg] {} did not return a function (returned type {})",
  ...
  static void runLoop(App* app, Context* bg) {
    debug::info("[bg] background thread entered run loop");
      ...
      while (auto msg = bg->inbox.pop()) {
        ...
        switch (msg->kind) {
          case dispatch::MessageKind::CommandCall: {
            ...
            std::string jsonArgs(msg->payload.data() + pipe1 + 1, pipe2 - pipe1 - 1);
            std::string callId(msg->payload.data() + pipe2 + 1, msg->payload.size() - pipe2 - 1);
            ...
            if (it == bg->commands->handlers.end()) {
              std::string errPayload = callId + "|" +
                                       ...
                                       cmdName + "'\"}";
              bg->outbox.push({dispatch::MessageKind::CommandResult, std::move(errPayload)});
              dispatch::notify(app);
              continue;
            }
            ...
            try {
              ...
              params    = common::toTable(lua, json);
            } catch (...) {
            ...
            if (result.valid()) {
              ...
              if (val.is<sol::table>()) {
                resultJson = common::toJson(val.as<sol::table>());
              ...
              nlohmann::json errJson = {{"code", "LuaError"}, {"message", err.what()}};
              resultPayload          = callId + "|" + errJson.dump();
            }

            bg->outbox.push({dispatch::MessageKind::CommandResult, std::move(resultPayload)});
            dispatch::notify(app);
            break;
          }

          default:
            debug::warn(std::format("[bg] unknown message kind: {}", static_cast<int>(msg->kind)));
            break;
        }
      }
    ...
    debug::info("[bg] background thread stopped");
  }
  ...
  void start(Context* bg) {
    ...
    bg->thread = std::thread([bg]() { runLoop(bg->app, bg); });
  }
