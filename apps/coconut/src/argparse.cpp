...
namespace coconut::argparse {
  ...
  static void setHelp(Args& a, const std::string&) {
    a.help = true;
  }
  static void setVersion(Args& a, const std::string&) {
    a.version = true;
  }
  static void setDebug(Args& a, const std::string&) {
    a.debug = true;
  }
  static void setBytecode(Args& a, const std::string&) {
    a.bytecode_config = true;
  }
  static void setWatch(Args& a, const std::string&) {
    a.watch = true;
  }
  static void setFrameless(Args& a, const std::string&) {
    a.override_frameless = true;
  }
  static void setTransparent(Args& a, const std::string&) {
    a.override_transparent = true;
  }
  static void setYes(Args& a, const std::string&) {
    a.yes = true;
  }

  static void setRoot(Args& a, const std::string& v) {
    a.root = v;
  }
  static void setOutDir(Args& a, const std::string& v) {
    a.out_dir = v;
  }
  static void setTemplate(Args& a, const std::string& v) {
    a.template_name = v;
  }
  static void setTitle(Args& a, const std::string& v) {
    a.override_title_given = true;
    a.override_title       = v;
  }
  static void setWinWidth(Args& a, const std::string& v) {
    a.override_window_width = std::atoi(v.c_str());
  }
  static void setWinHeight(Args& a, const std::string& v) {
    a.override_window_height = std::atoi(v.c_str());
  }

  // Subcommand setters
  static void setGenerate(Args& a) {
    a.generate = true;
  }
  static void setBundle(Args& a) {
    a.bundle = true;
  }
  static void setNew(Args& a) {
    a.new_cmd = true;
  }
  static void setRun(Args& a) {
    a.run_cmd = true;
  }
  ...
  static const Subcommand SUBCOMMANDS[] = {
      ...
      {"generate",
       "Generate command wrappers from @command annotations",
       setGenerate,
       printGenerateHelp},
  ...
  Args parse(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (!opt) {
        ...
        // Print relevant help
        for (const auto& sc : SUBCOMMANDS) {
          if (sc.apply == setGenerate && args.generate) {
            sc.printHelp(argv[0]);
            break;
          }
          if (sc.apply == setBundle && args.bundle) {
            sc.printHelp(argv[0]);
            break;
          }
          if (sc.apply == setNew && args.new_cmd) {
            sc.printHelp(argv[0]);
            break;
          }
          if (sc.apply == setRun && args.run_cmd) {
            sc.printHelp(argv[0]);
            break;
          }
        }
  ...
  void printBundleHelp(const char* prog) {
    ...
    coconut::println(
        "  3. Generates platform manifests (Info.plist, app.manifest, .desktop, metainfo.xml)"
    );
