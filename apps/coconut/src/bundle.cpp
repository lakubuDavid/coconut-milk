...
namespace coconut::bundle {
  ...
  static bool copyFile(const std::string& src, const std::string& dst, std::string& error_out) {
    std::error_code ec;
    std::filesystem::create_directories(std::filesystem::path(dst).parent_path(), ec);
    if (ec) {
      error_out = std::format("failed to create parent dir for '{}': {}", dst, ec.message());
      return false;
    }
    if (!std::filesystem::copy_file(
            src, dst, std::filesystem::copy_options::overwrite_existing, ec
        )) {
      error_out = std::format("failed to copy '{}' -> '{}': {}", src, dst, ec.message());
      return false;
    }
    return true;
  }
  ...
  static bool copyDir(
      const std::string& src_dir, const std::string& dst_dir, std::string& error_out
  ) {
    std::error_code ec;
    ...
    for (auto& entry : std::filesystem::recursive_directory_iterator(src_dir, ec)) {
      ...
      if (entry.is_directory()) {
        ...
        std::filesystem::copy_file(
            src_path, dst_path, std::filesystem::copy_options::overwrite_existing, ec
        );
      }
      if (ec) {
  ...
  template <typename F>
  static std::string resolveAppField(const PlatformConfig& p, const AppConfig& shared, F&& getter) {
    auto v = getter(p.app);
    if (!v.empty())
      return v;
  ...
  static std::string resolveBundleId(const Config& cfg, const PlatformConfig& p) {
    if (!p.bundle_identifier.empty())
      return p.bundle_identifier;
    if (!p.app.id.empty())
      return p.app.id;
    return cfg.app.id;
  }
  ...
  static void plistString(std::ofstream& f, const std::string& key, const std::string& val) {
    f << "  <key>" << key << "</key>\n"
      << "  <string>" << val << "</string>\n";
  }
  ...
  static bool writeInfoPlist(
    ...
    std::string app_name  = resolveAppField(d, cfg.app, [](const AppConfig& a) { return a.name; });
    std::string app_version =
        resolveAppField(d, cfg.app, [](const AppConfig& a) { return a.version; });
    std::string category =
        resolveAppField(d, cfg.app, [](const AppConfig& a) { return a.category; });
    ...
    // Usage descriptions
    for (const auto& [key, val] : d.ns.usage_descriptions) plistString(f, key, val);

    // Extra Info.plist keys from manifests
    for (const auto& [key, val] : cfg.manifests.darwin_info_plist_extra) plistString(f, key, val);
  ...
  static bool writeEntitlements(
    ...
    if (cfg.manifests.darwin_entitlements.empty())
    ...
    f << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
      ...
      << "<plist version=\"1.0\">\n"
      << "<dict>\n";
    for (const auto& [key, val] : cfg.manifests.darwin_entitlements) plistString(f, key, val);
    f << "</dict>\n</plist>\n";
    f.close();
    return true;
  }
  ...
  static bool writeWindowsManifest(
    ...
    std::string app_name = resolveAppField(w, cfg.app, [](const AppConfig& a) { return a.name; });
  ...
  static bool writeDesktopFile(
    ...
    std::string app_name = resolveAppField(l, cfg.app, [](const AppConfig& a) { return a.name; });
    std::string app_desc =
        resolveAppField(l, cfg.app, [](const AppConfig& a) { return a.description; });
    std::string app_id = resolveAppField(l, cfg.app, [](const AppConfig& a) { return a.id; });
    if (app_id.empty())
      app_id = "coconut-app";
    ...
    for (const auto& [key, val] : cfg.manifests.linux_desktop_extra) f << key << "=" << val << "\n";
  ...
  static bool writeMetainfo(const Config& cfg, const std::string& out_dir, std::string& error_out) {
    const auto& l        = cfg.linux;
    std::string app_name = resolveAppField(l, cfg.app, [](const AppConfig& a) { return a.name; });
    std::string app_desc =
        resolveAppField(l, cfg.app, [](const AppConfig& a) { return a.description; });
  ...
  std::expected<std::string, Error> writeShippableConfig(
    ...
    // app identity
    if (!shippable.app.name.empty())
      j["app"]["name"] = shippable.app.name;
    if (!shippable.app.id.empty())
      j["app"]["id"] = shippable.app.id;
    if (!shippable.app.version.empty())
      j["app"]["version"] = shippable.app.version;
    if (!shippable.app.description.empty())
      j["app"]["description"] = shippable.app.description;
    if (!shippable.app.category.empty())
      j["app"]["category"] = shippable.app.category;

    // icon paths
    if (!shippable.icon.source.empty())
      j["icon"]["source"] = shippable.icon.source;
    if (!shippable.icon.icns_path.empty())
      j["icon"]["icns_path"] = shippable.icon.icns_path;
    if (!shippable.icon.ico_path.empty())
      j["icon"]["ico_path"] = shippable.icon.ico_path;
    if (!shippable.icon.png_path.empty())
      j["icon"]["png_path"] = shippable.icon.png_path;
    ...
    auto writePlatform = [&](const std::string& key, const PlatformConfig& p) {
      if (p.frameless.has_value())
        j[key]["frameless"] = p.frameless.value();
      if (p.transparent.has_value())
        j[key]["transparent"] = p.transparent.value();
      if (!p.app.name.empty())
        j[key]["app"]["name"] = p.app.name;
      if (!p.app.id.empty())
        j[key]["app"]["id"] = p.app.id;
      if (!p.bundle_identifier.empty())
        j[key]["bundle_identifier"] = p.bundle_identifier;
      ...
      for (const auto& [k, v] : p.ns.usage_descriptions) j[key]["ns"]["usage_descriptions"][k] = v;
    };
  ...
  std::expected<std::string, Error> generateManifests(
    ...
    if (!writeInfoPlist(cfg, out_dir, error)) {
      return std::unexpected(Error{
          .code = ErrorCode::IoError, .message = "generate manifests: " + error});
    }
  ...
  std::expected<std::string, Error> assembleBundle(const Config& cfg, const std::string& out_dir) {
    ...
    if (self_path.empty()) {
      return std::unexpected(Error{
          .code = ErrorCode::Unknown, .message = "assemble: could not find running executable path"}
      );
    }
    ...
    if (!copyFile(self_path, bin_dst, error)) {
      return std::unexpected(Error{.code = ErrorCode::IoError, .message = "assemble: " + error});
    }
    ...
    if (std::filesystem::exists(out_dir + "/coconut.config.json")) {
      std::filesystem::rename(
          out_dir + "/coconut.config.json", res_dir + "/coconut.config.json", ec
      );
      (void)ec;
    }
  ...
  std::expected<std::string, Error> compileLuaFile(const std::string& lua_path) {
    ...
    try {
      sol::state lua;
      lua.open_libraries(
          sol::lib::jit, sol::lib::base, sol::lib::string, sol::lib::io, sol::lib::os
      );
      ...
      if (!script.valid()) {
        return std::unexpected(Error{
            .code = ErrorCode::LuaError, .message = "failed to load bytecode compilation script"});
      }
  ...
  std::expected<std::string, Error> bundle(
      const Config& cfg, const std::string& out_dir, bool bytecode_flag
  ) {
    ...
    if (!stripped) {
      return std::unexpected(Error{
          ...
          .message =
              std::format("bundle: failed to write shippable config: {}", stripped.error().message),
    ...
    std::string app_id =
        resolveAppField(cfg.darwin, cfg.app, [](const AppConfig& a) { return a.id; });
    if (app_id.empty())
      app_id = "coconut-app";
    {
      // Only skip icon generation if user explicitly provided all three paths
      bool hasExplicitPaths =
          !cfg.icon.icns_path.empty() || !cfg.icon.ico_path.empty() || !cfg.icon.png_path.empty();
      if (!hasExplicitPaths) {
        auto icons = icon_gen::generateIcons(cfg.icon.source, out_dir, app_id);
        if (!icons) {
          std::cerr << "bundle: warning: icon generation failed: " << icons.error().message
                    << std::endl;
        } else {
          std::cout << "bundle: icons generated (icns="
                    << (icons->icns.empty() ? "none" : icons->icns)
                    << ", ico=" << (icons->ico.empty() ? "none" : icons->ico)
                    << ", png=" << (icons->png.empty() ? "none" : icons->png) << ")" << std::endl;
        }
      }
    }
    ...
    if (!manifests) {
      return std::unexpected(Error{
          ...
          .message =
              std::format("bundle: manifests generation failed: {}", manifests.error().message)});
    }
    ...
    if (!assembled) {
      return std::unexpected(Error{
          ...
          .message = std::format("bundle: assembly failed: {}", assembled.error().message)});
    }
    ...
    if (bytecode_flag) {
      ...
      if (compileResult) {
        std::cout << "bundle: compiled " << compileResult.value() << " Lua file(s) to bytecode"
                  << std::endl;
      } else {
