...
std::optional<coconut::Error>
bindLuaContext(std::function<void(sol::state *)> callback);
...
std::optional<coconut::Error> shutdownWorker(
    ...
    std::chrono::milliseconds softAbortTimeout = std::chrono::milliseconds{
        30000});
...
using WorkerInitializer =
    std::function<std::optional<coconut::Error>(Worker *)>;
...
struct WorkerPool {
  ...
  std::optional<coconut::Error> queueMessage(const std::string &command,
                                            nlohmann::json args);
