...
namespace coconut::core {
  ...
  std::expected<WorkerPtr, Error> createWorker() {
    ...
    worker->LuaState->open_libraries(
        sol::lib::string, sol::lib::base, sol::lib::table, sol::lib::package, sol::lib::os
    );
  ...
  std::optional<Error> bindModules(Worker *worker, modules::ModulesFlag modules) {
    if (!worker || !worker->LuaState)
      return Error{.code = ErrorCode::LuaError, .message = "the worker's lua state not ready"};
  ...
  std::optional<coconut::Error> bindLuaContext(
      Worker *worker, std::function<void(sol::state *)> callback
  ) {
    try {
    ...
    while (!worker->StopRequested.load(std::memory_order_acquire)) {
      ...
      std::visit(
          [worker, lua](const PromiseMessage &req) {
            CoconutContext *ctx = worker->Context;
  ...
  std::optional<coconut::Error> shutdownWorker(
    ...
    if (flag == Worker::ShutdownFlag::SoftAbort) {
      ...
      while (worker->isRunning() && std::chrono::steady_clock::now() < hardDeadline) {
        std::this_thread::yield();
      }
  ...
  std::expected<std::unique_ptr<WorkerPool>, coconut::Error> createWorkerPool(int size) {
    if (size <= 0) {
      return std::unexpected(Error{.message = "createWorkerPool: size must be > 0"});
    }
  ...
  WorkerPoolBuilder &WorkerPoolBuilder::withModules(coconut::modules::ModulesFlag modules) {
  ...
  std::optional<coconut::Error> WorkerPool::shutdownAll(Worker::ShutdownFlag flag) {
    for (auto &w : Workers) {
      if (w) {
