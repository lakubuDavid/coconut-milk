...
namespace coconut::modules {
  ...
  void init_bridge_emit(sol::state& lua, ThreadKind kind) {
    ...
    if (kind != ThreadKind::Main) {
      ...
      coconut.set_function("_bridge_emit", [](const std::string&, const std::string&) {
        debug::warn("[bg] _bridge_emit requires the main thread");
      });
      coconut.set_function("_js_log", [](const sol::table&) {
    ...
    coconut.set_function(
        "_bridge_emit",
        [](sol::this_state s, const std::string& name, const std::string& payloadJson) {
          ...
          try {
            ...
            debug::warn(std::format("_bridge_emit: failed to parse payload: {}", e.what()));
          }
        }
    );

    // ── JS error forwarding ─────────────────────────────────────
    coconut.set_function("_js_log", [](sol::this_state, const sol::table& entry) {
