#include "env.h"
#include "../core/env.h"  // C++ env namespace

namespace coconut::modules {

  void init_env(sol::state& lua, ThreadKind kind) {
    if (kind == ThreadKind::Background)
      return;  // not available on bg
    ...
    mt["__index"] = [&lua](sol::table, const std::string& key) -> sol::object {
      ...
      if (key == "cwd") {
        return sol::make_object(lv.lua_state(), coconut::core::env::cwd());
      }
      if (key == "homedir") {
        return sol::make_object(lv.lua_state(), coconut::core::env::homedir());
      }
      if (key == "pathSeparator") {
        return sol::make_object(
            lv.lua_state(), std::string(1, coconut::core::env::pathSeparator())
        );
      }
      std::string val = coconut::core::env::get(key);
      if (val.empty())
        return sol::nil;
