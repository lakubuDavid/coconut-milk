...
void init_keybind(sol::state& lua, ThreadKind kind) {
  ...
  if (kind != ThreadKind::Main) {
    ...
    coconut.set_function("keybind",
        [](sol::this_state, const std::string&, sol::protected_function,
           sol::optional<sol::table>) -> sol::function {
          debug::warn("[bg] coconut.keybind requires the main thread");
          return sol::lua_nil;
        });
    coconut.set_function("__registerPlatformKeybind",
        [](sol::table) -> bool {
  ...
  coconut.set_function("__registerPlatformKeybind",
      [](sol::this_state s, sol::table params) -> bool {
        ...
        debug::info(
            std::format("[keybind] registered platform keybind: {}", combo));
        return true;
      });
  ...
  coconut.set_function("keybind",
      [](sol::this_state s, const std::string& combo,
        ...
        // Build entry table.
        std::string id =
            opts_tbl ? opts_tbl.value()["id"].get_or(combo) : combo;
        std::string scope =
            opts_tbl
                ? opts_tbl.value()["scope"].get_or(std::string("global"))
                : "global";
        bool platform =
            opts_tbl ? opts_tbl.value()["platform"].get_or(false) : false;
