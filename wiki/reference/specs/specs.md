...
title: Specification (v0 API)

parent: Specifications

nav_order: 1

description: The current minimal Lua desktop UI framework API specification.
...
> 🕰️ **Historical design specification.** Written before implementation; the
> shipped code has evolved (e.g. `rpc::Message` → `core::JsRPCMessage`,
> `dispatch::Outbox` → `core::Dispatcher`, `bg_thread` → `core::WorkerPool`,
> uniform async replies via `__coconut_rpc_receive`). Consult
> `docs/architecture/` and `wiki/concepts/bridge-protocol.md` for current
> behavior; keep this document for design rationale.
...
Acts as the last-resort fallback in the event dispatch chain.



- called for every event that reaches the end of the dispatch chain

- `event` is an event object (see [Event object model](#5-event-model) for shape)

- fires after all `view:on()` and `coconut.on()` callbacks



View controller callbacks are attached via methods on the view descriptor:
...
- `view:on(name, fn)` - when an event is emitted while the view is active
...
ctx:emit({ name = "toast", message = "saved" })

ctx:emit_sync({ name = "ping", at = os.time() })
...
Lifecycle events are emitted by the platform runtime and flow through

the standard dispatch chain. Subscribe to them with `coconut.on()`:
...
coconut.on("resize", function(event)
  ...
  print("window resized to", event.w, "x", event.h)
...
coconut.on("close", function(event)
  ...
  -- Call event:preventDefault() to veto close
...
Views can subscribe to lifecycle events. The view callback fires
...
function coconut.config(ctx)
  ...
  ctx:setWindowSize({ w = 1280, h = 640 })
     ...
     :bind("sayHi", function(params, ctx)
       ...
       ctx:emit({ name = "greeted", user = params.name or "user" })
  ...
  coconut.on("resize", function(event)
    ...
    -- runtime provides event so Lua can react
...
- `ctx:emit(event)` — Lua → frontend

- `coconut.emit(event)` — frontend → Lua
...
await coconut.emit({ name: "frontend_ready", at: Date.now() })
...
- accepts a single event object (must have a `name` field)

- async, returns a `Promise<void>`
...
- `event` is a JS object with a string `name` field and optional payload fields
...
#### `coconut.on(name, fn)` with options

```ts

coconut.on("resize", (event) => { ... }, { once: true })
...
- `ctx:emit(event)` — single event object

- `ctx:emit_sync(event)` — single event object
...
- frontend `coconut.on(name, fn)` listener API (returns unregister)

- frontend `coconut.emit(event)` signaling API (object form)

- `coconut.on(event, fn, {once=true})` subscribe API

- `view:on(name, fn)` view-scoped event callbacks

- DOM-like event objects with `preventDefault()` / `stopPropagation()`

- three-tier dispatch chain: view → subscribe → fallback

- lifecycle events (`resize`, `focus`, `blur`, `ready`, `close`)
