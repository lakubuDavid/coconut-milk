...
COCONUT_TEST(unit, argparse_help_flag) {
  ...
  auto args = coconut::argparse::parse(2, const_cast<char**>(argv));
...
COCONUT_TEST(unit, argparse_generate_with_watch) {
  ...
  auto args = coconut::argparse::parse(3, const_cast<char**>(argv));
  COCONUT_REQUIRE(args.generate);
...
COCONUT_TEST(unit, argparse_generate_with_outdir) {
  ...
  auto args = coconut::argparse::parse(4, const_cast<char**>(argv));
  COCONUT_REQUIRE(args.generate);
...
COCONUT_TEST(unit, argparse_new_with_template) {
  ...
  auto args = coconut::argparse::parse(5, const_cast<char**>(argv));
  COCONUT_REQUIRE(args.new_cmd);
  COCONUT_REQUIRE_EQ(args.new_name, std::string("myapp"));
...
COCONUT_TEST(unit, argparse_run_with_override_flags) {
  const char* argv[] = {"coconut", "run", "--window-width", "1024", "--window-height", "768",
                        "--title", "My App", "--frameless", "--debug", nullptr};
  auto args = coconut::argparse::parse(10, const_cast<char**>(argv));
  COCONUT_REQUIRE(args.run_cmd);
...
COCONUT_TEST(unit, argparse_defaults) {
  ...
  auto args = coconut::argparse::parse(1, const_cast<char**>(argv));
  COCONUT_REQUIRE(!args.help);
  COCONUT_REQUIRE(!args.version);
  COCONUT_REQUIRE(!args.debug);
  COCONUT_REQUIRE(!args.generate);
  ...
  COCONUT_REQUIRE(args.key_value_args.empty());
  COCONUT_REQUIRE(args.flag_args.empty());
}
...
// ── Key-value arguments (not yet implemented by parse) ───────────────

COCONUT_TEST(unit, argparse_key_value_args_not_yet) {
  // key_value_args is declared in Args but not populated by parse().
  const char* argv[] = {"coconut", "key=value", "name=test", nullptr};
  auto args = coconut::argparse::parse(3, const_cast<char**>(argv));
  COCONUT_REQUIRE(args.key_value_args.empty());
}
