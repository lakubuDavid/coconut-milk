...
The diagram below shows how messages flow between Lua and JavaScript through the bridge:

<p align="center">
  <img src="../diagrams/bridge-communication.png" alt="Bridge communication flow diagram" width="800">
</p>
...
> See [generators.md](specs/generators.md).
...
```
1. Lua backend:
   ctx:emit({ name = "toast", message = "Saved!", type = "success" })
...
```lua
-- Lua side:
ctx:emit({ name = "event", ... })  -- 100th event while not ready
-- → Returns false, logs "QueueOverflow" error
```

```js
// Frontend side:
await coconut.emit({ name = "event", ... })  // While not ready
...
```typescript
await coconut.emit(event: { name: string, [key: string]: any }): Promise<void>
```
...
- Read **[Troubleshooting](../explanation/troubleshooting.md)** for common issues
