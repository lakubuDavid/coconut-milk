...
| `on` | When frontend emits a matching event | Handle navigation, toasts |
...
1. **No file is served** — Instead, JavaScript is evaluated in the webview:
   ```js
   coconut.emit({name:'navigate', view:'settings'})
   ```
...
|---|---|---|
| `ctx:emit(event)` | Async, queue-based, non-blocking | Normal events (toasts, updates) |
...
ctx:emit({ name = "click", x = 10 })     -- Event namespace
```
...
- Read the **[Bridge (Advanced)](../reference/bridge.md)** for protocol details
